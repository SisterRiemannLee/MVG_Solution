...
% (a)
% transform the model by the vector T using homogeneous coordinates and a
% 3x4 rigid body transformation
...
% R(4*N) with last column being all ones
...
% camera intrinsics
% translate image coordinates to pixel coordinates
K = [540 0 320; ...
     0 540 240; ...
     0   0   1];
...
% parallel中就不需要除z了，orthographic projection 
...
% handle = @(arglist)anonymous_function: 也成为匿名函数，@左边为函数句柄，
% @后定义了匿名函数的输入参数（多个参数用逗号分隔开），最后一部分为匿名函
% 数的表达式。
